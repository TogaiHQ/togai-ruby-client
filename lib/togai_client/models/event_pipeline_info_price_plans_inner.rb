...
module TogaiClient
  class EventPipelineInfoPricePlansInner
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::EventPipelineInfoPricePlansInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::EventPipelineInfoPricePlansInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'cycle_end')
        self.cycle_end = attributes[:'cycle_end']
      end
    ...
    def list_invalid_properties
      ...
      if @cycle_end.nil?
        invalid_properties.push('invalid value for "cycle_end", cycle_end cannot be nil.')
      end
    ...
    # @param [Object] usage_meters Value to be assigned
    def usage_meters=(usage_meters)
