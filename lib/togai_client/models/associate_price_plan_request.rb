...
module TogaiClient
  ...
  class AssociatePricePlanRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::AssociatePricePlanRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::AssociatePricePlanRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'price_plan_name')
    ...
    def list_invalid_properties
      ...
      if @price_plan_name.nil?
        invalid_properties.push('invalid value for "price_plan_name", price_plan_name cannot be nil.')
      end

      if @price_plan_name.to_s.length > 50
    ...
    # @param [Object] price_plan_name Value to be assigned
    def price_plan_name=(price_plan_name)
      if price_plan_name.nil?
        fail ArgumentError, 'price_plan_name cannot be nil'
      end

      if price_plan_name.to_s.length > 50
        fail ArgumentError, 'invalid value for "price_plan_name", the character length must be smaller than or equal to 50.'
      end
