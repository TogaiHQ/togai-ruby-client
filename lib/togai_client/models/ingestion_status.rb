  ...
  class IngestionStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::IngestionStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::IngestionStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@status_description.nil? && @status_description.to_s.length > 250
        invalid_properties.push('invalid value for "status_description", the character length must be smaller than or equal to 250.')
      end
    ...
    def status_description=(status_description)
      if !status_description.nil? && status_description.to_s.length > 250
        fail ArgumentError, 'invalid value for "status_description", the character length must be smaller than or equal to 250.'
      end
